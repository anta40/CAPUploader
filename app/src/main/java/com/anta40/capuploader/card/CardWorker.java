package com.anta40.capuploader.card;
...
import com.anta40.capuploader.card.cap.structure.Applet;
import com.anta40.capuploader.card.cap.structure.Header;
import com.anta40.capuploader.card.cap.structure.StaticField;
import com.anta40.capuploader.card.cap.utils.CAPToIJCInputStream;
...
//			print("Encrypted HostCryptoGram is " + ByteUtils.htos(calculatedHostCryptogram));
			...
			respAPDU = cardTransmit(extauthAPDU, 0, extauthAPDU.length); // cardTransmit = null disini. wonder why...
			 ...
			 * P1: xx
			 * P2: 00
			 * Lc: 10 (length of cryptogram and MAC)
			 * Data: xx xx (host cryptogram and MAC)
			 */
			
			edt.append("authenticate_SCP02 result: \n");
			edt.append(ByteUtils.htos(respAPDU));
...
//		byte[] issuerDomain=ByteUtils.stoh(Settings.props.getProperty("CMAID"));
		byte[] issuerDomain = ByteUtils.stoh("A000000003000000");
		 ...
		 * INITIALIZE UPDATE
		 * 
		 * Menyiapkan secure channel session utk pertukaran data antara kartu dgn "host" (off-card entity)
		 * 
		 * Format perintah:
		 * CLA: 80-83 atau C0-FF
...
//		print("*************");
	...
	//	respAPDU = cardTransmit(selectAPDU, 0, selectAPDU.length);
		...
		if (auth) {
			...
			edt.append("Authenticated\n");
			return true;
		} else {
			//alwaysPrint("Could not authenticate. Aborting.");
			edt.append("Could not authenticate. Aborting.\n");
			return false;
		}
	}
...
//			card.disconnect(true);
	...
	public byte[] generateMAC_SCP02(byte[] iv, byte[] data) throws Exception {
		...
		Cipher singleDesCipher = Cipher.getInstance("DES/CBC/NoPadding", "SC");
...
//			print("Applet AID:" + ByteUtils.htos(appletAID));
			...
			//hardcoded values!!!
//			appletAID = new byte[]{(byte) 0xA0, (byte) 0x00,(byte) 0x00,(byte) 0x00,(byte) 0x04,(byte) 0x10,(byte) 0x10};
//			capAID = new byte[]{(byte) 0xA0, (byte) 0x00, (byte) 0x00,  (byte) 0x00, (byte) 0x04, (byte) 0x10}; 
...
//			print("Install for Load");
			...
			// masuk sini ada IOException: while sending APDU tag was lost
			// timeout: 10000
			if (SCP == 1)
...
//			print("Load CAP");
//			String cap = capFile.getAbsolutePath();
			if (!capFile.isFile()) return;
			CAPToIJCInputStream fis = new CAPToIJCInputStream(capFile.getAbsolutePath());
			...
			respAPDU = cardTransmit(registerAPDU, 0, registerAPDU.length); // kok respAPDU =  null disini??
		...
		if (allOK){
			...
			edt.append("Applet loaded & registered\n");
			Log.e("CAPUploder", "Applet loaded & registered");
		}
		else{
			//alwaysPrint("Could not load applet. See debug for more info");
			edt.append("Could not load applet\n");
			Log.e("CAPUploader","Could not load applet. See debug for more info");
		}
...
//			System.out.println(s);
		edt.append(s);
		edt.append(System.getProperty("line.separator"));
	}
		...
		try {
			...
			for(Enumeration enumeration = jarfile.entries(); enumeration.hasMoreElements();)
			{
				...
				if(jarentry.getName().lastIndexOf(capFiles[3]) >= 0)
				{
					...
					appletAID = applet.getAID(0).getAid(); // kenapa null???
				}
...
//	}
	...
	public byte[] cardTransmit(byte[] apdu, int p, int apduLength){
		byte[] resp = null;
		
		try {
			resp = isodep.transceive(apdu);
			edt.append("APDU: "+ByteUtils.htos(apdu)+"\n");
			edt.append("Response: "+ByteUtils.htos(resp)+"\n");
		}
		catch (IOException ioe){
			Log.e("CAPUploader","IOException while sending APDU: "+ioe.getMessage());
		}
		return resp;
	...
	public void setTextArea(EditText ed){
		this.edt = ed;
	}
